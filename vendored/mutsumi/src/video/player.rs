...
use crate::MPVGLArea;
use crate::{ContextedMPV, MpvValue, PlayParams};
...
mod imp {
    ...
    impl ObjectImpl for MutsumiVideoPlayer {
        fn constructed(&self) {
            ...
            #[cfg(not(target_os = "linux"))]
            {
                // Render-API path: embed the GL area directly and let libmpv
                // draw into it (requires vo=libmpv).
                self.backend.set_hexpand(true);
                self.backend.set_vexpand(true);
                obj.set_child(Some(&self.backend));
                self.backend.set_property("vo", "libmpv".to_owned());
            }
...
impl MutsumiVideoPlayer {
    ...
    #[cfg(not(target_os = "linux"))]
    pub fn backend_ref(&self) -> &MPVGLArea {
        let imp = self.imp();
        &imp.backend
    }
