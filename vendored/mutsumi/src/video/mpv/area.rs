...
mod imp {
    ...
    impl ObjectSubclass for MPVGLArea {
        const NAME: &'static str = "MutsumiMPVGLArea";
...
impl MPVGLArea {
    ...
    // No-op: the GL area repaints from the render context, so there is no
    // stale dmabuf texture to clear when playback stops.
